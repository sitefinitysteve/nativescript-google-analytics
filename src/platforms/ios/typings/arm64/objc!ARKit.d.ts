...
declare class ARCamera extends NSObject implements NSCopying {
	...
	unprojectPointOntoPlaneWithTransformOrientationViewportSize(point: CGPoint, planeTransform: simd_float4x4, orientation: UIInterfaceOrientation, viewportSize: CGSize);
...
declare class AREnvironmentProbeAnchor extends ARAnchor {
	...
	constructor(o: { name: string; transform: simd_float4x4; extent; });

	constructor(o: { transform: simd_float4x4; extent; });

	initWithNameTransformExtent(name: string, transform: simd_float4x4, extent ): this;

	initWithTransformExtent(transform: simd_float4x4, extent ): this;
}
...
declare class ARRaycastQuery extends NSObject {
	...
	constructor(o: { origin; direction; allowingTarget: ARRaycastTarget; alignment: ARRaycastTargetAlignment; });

	initWithOriginDirectionAllowingTargetAlignment(origin, direction, target: ARRaycastTarget, alignment: ARRaycastTargetAlignment): this;
}
...
declare class ARSCNView extends SCNView implements ARSessionProviding {
	...
	unprojectPointOntoPlaneWithTransform(point: CGPoint, planeTransform: simd_float4x4);
}
...
declare class ARSession extends NSObject {
	...
	createReferenceObjectWithTransformCenterExtentCompletionHandler(transform: simd_float4x4, center, extent, completionHandler: (p1: ARReferenceObject, p2: NSError) => void): void;

	getCurrentWorldMapWithCompletionHandler(completionHandler: (p1: ARWorldMap, p2: NSError) => void): void;

	getGeoLocationForPointCompletionHandler(position, completionHandler: (p1: CLLocationCoordinate2D, p2: number, p3: NSError) => void): void;
...
declare class ARSkeleton2D extends ARSkeleton {
	...
	readonly jointLandmarks: interop.Pointer | interop.Reference<>;

	landmarkForJointNamed(jointName: string);
}
