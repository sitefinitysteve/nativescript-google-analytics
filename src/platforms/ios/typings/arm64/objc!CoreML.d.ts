...
declare class MLArrayBatchProvider extends NSObject implements MLBatchProvider {
	...
	constructor(o: { featureProviderArray: NSArray<MLFeatureProvider>; });

	featuresAtIndex(index: number): MLFeatureProvider;

	initWithDictionaryError(dictionary: NSDictionary<string, NSArray<any>>): this;

	initWithFeatureProviderArray(array: NSArray<MLFeatureProvider>): this;
}
...
interface MLCustomLayer {
	...
	encodeToCommandBufferInputsOutputsError?(commandBuffer: MTLCommandBuffer, inputs: NSArray<MTLTexture>, outputs: NSArray<MTLTexture>): boolean;

	evaluateOnCPUWithInputsOutputsError(inputs: NSArray<MLMultiArray>, outputs: NSArray<MLMultiArray>): boolean;

	initWithParameterDictionaryError?(parameters: NSDictionary<string, any>): MLCustomLayer;

	outputShapesForInputShapesError(inputShapes: NSArray<NSArray<number>>): NSArray<NSArray<number>>;

	setWeightDataError(weights: NSArray<NSData>): boolean;
}
...
declare class MLMultiArray extends NSObject implements NSSecureCoding {
	...
	static multiArrayByConcatenatingMultiArraysAlongAxisDataType(multiArrays: NSArray<MLMultiArray>, axis: number, dataType: MLMultiArrayDataType): MLMultiArray;
...
declare class MLSequence extends NSObject implements NSSecureCoding {
	...
	static sequenceWithInt64Array(int64Values: NSArray<number>): MLSequence;

	static sequenceWithStringArray(stringValues: NSArray<string>): MLSequence;
