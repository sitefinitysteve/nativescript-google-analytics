...
declare class NLEmbedding extends NSObject {
	...
	enumerateNeighborsForVectorMaximumCountDistanceTypeUsingBlock(vector: NSArray<number>, maxCount: number, distanceType: NLDistanceType, block: (p1: string, p2: number, p3: interop.Pointer | interop.Reference<boolean>) => void): void;

	enumerateNeighborsForVectorMaximumCountMaximumDistanceDistanceTypeUsingBlock(vector: NSArray<number>, maxCount: number, maxDistance: number, distanceType: NLDistanceType, block: (p1: string, p2: number, p3: interop.Pointer | interop.Reference<boolean>) => void): void;
	...
	neighborsForVectorMaximumCountDistanceType(vector: NSArray<number>, maxCount: number, distanceType: NLDistanceType): NSArray<string>;

	neighborsForVectorMaximumCountMaximumDistanceDistanceType(vector: NSArray<number>, maxCount: number, maxDistance: number, distanceType: NLDistanceType): NSArray<string>;
...
declare class NLModel extends NSObject {
	...
	predictedLabelHypothesesForTokensMaximumCount(tokens: NSArray<string>, maximumCount: number): NSArray<NSDictionary<string, number>>;

	predictedLabelsForTokens(tokens: NSArray<string>): NSArray<string>;
}
...
declare class NLTagger extends NSObject {
	...
	constructor(o: { tagSchemes: NSArray<string>; });
	...
	initWithTagSchemes(tagSchemes: NSArray<string>): this;

	modelsForTagScheme(tagScheme: string): NSArray<NLModel>;

	setGazetteersForTagScheme(gazetteers: NSArray<NLGazetteer>, tagScheme: string): void;

	setLanguageRange(language: string, range: NSRange): void;

	setModelsForTagScheme(models: NSArray<NLModel>, tagScheme: string): void;
