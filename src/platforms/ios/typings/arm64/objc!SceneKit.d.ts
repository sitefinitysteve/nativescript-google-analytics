...
declare class SCNHitTestResult extends NSObject {
	...
	readonly simdLocalCoordinates;

	readonly simdLocalNormal;

	readonly simdModelTransform: simd_float4x4;

	readonly simdWorldCoordinates;

	readonly simdWorldNormal;
...
declare class SCNNode extends NSObject implements NSCopying, NSSecureCoding, SCNActionable, SCNAnimatable, SCNBoundingVolume, UIFocusItem {
	...
	readonly simdWorldRight;

	simdWorldTransform: simd_float4x4;

	readonly simdWorldUp;
	...
	static readonly simdLocalFront;

	static readonly simdLocalRight;

	static readonly simdLocalUp;
	...
	simdConvertPositionFromNode(position, node: SCNNode);

	simdConvertPositionToNode(position, node: SCNNode);
	...
	simdConvertVectorFromNode(vector, node: SCNNode);

	simdConvertVectorToNode(vector, node: SCNNode);

	simdLocalRotateBy(rotation: simd_quatf): void;

	simdLocalTranslateBy(translation): void;

	simdLookAt(worldTarget): void;

	simdLookAtUpLocalFront(worldTarget , worldUp, localFront): void;

	simdRotateByAroundTarget(worldRotation: simd_quatf, worldTarget): void;
